...
const CourseInvoice: React.FC<any> = ({
  ...
  const generateInvoice = () => {
    ...
    // Course Details Table
    const totalAmount = finalPrice || courseData?.price || 0
    const subtotal = totalAmount / 1.18
    const gst = totalAmount - subtotal
    const formattedAmount = formatCurrency(totalAmount)
    ...
    // If coupon applied, show original price and discount
    if (couponCode && discountAmount > 0) {
      const originalSubtotal = originalPrice / 1.18
      // const originalGst = originalPrice - originalSubtotal

      totalsData.push(
        ["Original Subtotal", formatCurrency(Math.floor(originalSubtotal))],
        [`Discount (${couponCode})`, `-${formatCurrency(Math.floor(discountAmount))}`],
        ["Subtotal after discount", formatCurrency(Math.floor(subtotal))],
        ["GST (18%)", formatCurrency(Math.floor(gst))],
        ["Total", formatCurrency(Math.floor(totalAmount))],
      )
    } else {
      // No coupon applied, show regular totals
      totalsData.push(
        ["Subtotal", formatCurrency(Math.floor(subtotal))],
        ["GST (18%)", formatCurrency(Math.floor(gst))],
        ["Total", formatCurrency(Math.floor(totalAmount))],
    ...
    doc.setFontSize(8)
    const pageWidth = doc.internal.pageSize.width // Get PDF page width
    const textWidth = doc.getTextWidth("This is a system-generated invoice and does not required a physical signature.")
    const xPosition = (pageWidth - textWidth) / 2 // Calculate centered X position
    doc.text(
      "This is a system-generated invoice and does not required a physical signature.",
      xPosition, // Dynamically centered X position
    ...
    // Save the invoice
    const invoiceFileName = `invoice-${safeText(invoiceNumber || Date.now())}.pdf`
    doc.save(invoiceFileName)
  }
